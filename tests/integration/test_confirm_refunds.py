...
def make_refund(client, event, order, payment, organizer, amount):
    ...
    client.post(
        url, {f"refund-{payment.id}": amount, "start-mode": "full", "perform": True}
    )
...
def make_payment_refund(
    payment_info, provider, get_request_order_payment, client, organizer
):
    order_kwargs = {
    ...
    refund = make_refund(
        client, order.event, order, payment, organizer, payment_info["amount"]
    )
...
TEST_REFUND_SUCCESSFUL_DATA = [
    {
        ...
        "hex_address": "0x1Cecc45BFa0045E0069153c8DbBC1CdEeC5Ea148",
    },  # Empty wallet
    {
        "currency": "DAI - Rinkeby",
        "amount": int("1000", base=10),
        "hex_address": "0x5dCa654ed4E8e22B993B813A0879506c8Db12791",
    },  # Empty wallet
]
...
def test_confirm_refund_successful(
    provider,
    event,
    ...
    for payment_info in TEST_REFUND_SUCCESSFUL_DATA:
        ...
        payment, refund = make_payment_refund(
            payment_info, provider, get_request_order_payment, client, organizer
        )
    ...
    with scopes_disabled():
        call_command("confirm_refunds", "--no-dry-run")
...
def test_confirm_refund_dry_run(
    provider,
    event,
    get_request_order_payment,
    create_admin_client,
...
def test_confirm_refunds_not_refunded_yet(
    provider,
    event,
    get_request_order_payment,
    create_admin_client,
