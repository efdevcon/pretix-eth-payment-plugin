...
def test_file_upload_success(admin_client, organizer, event, wallet_address):
    ...
    assert (
        b"3 new addresses that don't already exist in the database" in response.content
    )

    response = admin_client.post(
        request_url + "confirm/", {"action": "confirm"}, follow=True
    )
...
def test_file_upload_length_short(admin_client, organizer, event):
    ...
    response = admin_client.post(
        request_url, {"wallet_addresses": invalid_file}, follow=True
    )
    assert response.status_code == 200
...
def test_file_upload_empty_file(admin_client, organizer, event):
    ...
    response = admin_client.post(
        request_url, {"wallet_addresses": empty_file}, follow=True
    )
    assert response.status_code == 200
...
def test_file_upload_no_addresses_in_file(admin_client, organizer, event):
    ...
    response = admin_client.post(
        request_url, {"wallet_addresses": no_address_file}, follow=True
    )
    assert response.status_code == 200
