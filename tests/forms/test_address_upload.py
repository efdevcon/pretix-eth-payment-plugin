...
def test_file_upload_success(client, organizer, event, env):
    existing_wallet_address = env
...
@pytest.mark.django_db
def test_file_upload_length_short(client, organizer, event, env):
    client.login(email='admin@pretix', password='admin')
    ...
    response = client.post(request_url, {'wallet_addresses': invalid_file}, follow=True)
    assert response.status_code == 200
    assert b'Syntax error on line 1:' in response.content


@pytest.mark.django_db
def test_file_upload_length_long(client, organizer, event, env):
    client.login(email='admin@pretix', password='admin')
...
@pytest.mark.django_db
def test_file_upload_one_address_per_line(client, organizer, event, env):
    client.login(email='admin@pretix', password='admin')
...
@pytest.mark.django_db
def test_file_upload_invalid_middle(client, organizer, event, env):
    client.login(email='admin@pretix', password='admin')
...
@pytest.mark.django_db
def test_file_upload_invalid_prefix(client, organizer, event, env):
    client.login(email='admin@pretix', password='admin')
...
@pytest.mark.django_db
def test_file_upload_empty_file(client, organizer, event, env):
    client.login(email='admin@pretix', password='admin')
...
@pytest.mark.django_db
def test_file_upload_no_addresses_in_file(client, organizer, event, env):
    client.login(email='admin@pretix', password='admin')
