...
@pytest.mark.django_db
def test_refund_wallets_filtered_correctly(event, organizer, create_admin_client,
                                           get_organizer_scope, get_order_and_payment):
    WalletAddress.objects.bulk_create(
        [WalletAddress(event=event, hex_address=address) for address in WALLET_ADDRESSES])
